...
**Credential masking.** Detected credentials are the one deliberate exception to capture-all. When the `credential_detected` detector matches an API key or token, its value is fully redacted before the event is written — no character of the secret reaches the trail. What persists is the credential's type (`anthropic_api_key`, `aws_access_key_id`, …) plus an HMAC-SHA256 fingerprint, keyed by a random salt created once per install (`~/Library/Application Support/xCLAUDE Gateway/audit-salt`). The type keeps the row recognizable; the fingerprint is irreversible, verifies nothing outside your machine, and lets you correlate the same credential across events and sources. The same salt keys both audit sources — wrapped MCP traffic and Claude Code — so one credential carries the same fingerprint wherever it appears, and masked events still correlate across sources. There is no toggle. Everything else is recorded as captured, including PII no detector masked (oversized values are size-truncated and flagged as such).
